...
An incomplete final SSE event distinguishes a safe continuation from an action that needs more resources:

- A result cap returns `checkpoint.resumable=true` with `resume_solution_offset`; the offset indexes the next point in the same deterministic divisor chunk.
- A divisor cap returns `checkpoint.resumable=true` with `resume_divisor_cursor`; adjacent cursor chunks enumerate disjoint positive-divisor indices. Mixed-radix decoding jumps directly to large cursors, so later chunks do not regenerate earlier divisors.
- Incomplete factorization returns `checkpoint.resumable=false`, the blocked `q`, and an instruction to raise `factor_limit` and restart that fiber at divisor cursor zero. Because the factorization changes, any partial exact hits may repeat.
- A fiber that consumes the hosted wall-time budget is killed and reaped. It is reported as non-resumable on the unchanged hosted budget, with the blocked `q` available for a local or distributed worker.

For a safe continuation, send every scalar in `checkpoint.request_params` unchanged along with the same equation, denominator constraints, and predicates. The first resumed fiber uses the supplied cursor; later fibers start at cursor zero. The completion certificate records whether the response began at a cursor and therefore requires its prior segment. This partitions a large numerator search into exact, reproducible chunks without silently treating a timeout or cap as completion.
